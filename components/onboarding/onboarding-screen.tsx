...
const slides: Slide[] = [
  {
    ...
    items: [
      ...
      { icon: Brain, text: "Учитывает оценки пользователя — каждая оценка повышает точность следующего ответа" },
      { icon: MapPin, text: "Содержит данные по регионам, ЛПР, портфелю Сбера и истории взаимодействий" },
  ...
  {
    title: "Ограничения",
    sub: "Границы применения системы",
    items: [
      { icon: XCircle, text: "Не заменяет решение руководителя — предоставляет структуру и факты", accent: true },
      { icon: XCircle, text: "Не формирует цифры без источника — при отсутствии данных указывает необходимость проверки базовой линии", accent: true },
      { icon: XCircle, text: "Не использует закрытую информацию — работает с данными, внесёнными в систему", accent: true },
      { icon: XCircle, text: "Не выполняет коммуникации — готовит материал, действия выполняются пользователем", accent: true },
      { icon: Shield, text: "Данные хранятся локально, передача третьим сторонам не производится" },
    ],
  },
  {
    ...
    items: [
      ...
      { icon: CheckCircle2, text: "2. Проверьте тип материала и регион — система подберёт контекст" },
      { icon: CheckCircle2, text: "3. Результат формируется за 2 минуты. Оценка ответа повышает точность системы" },
    ],
  },
];
