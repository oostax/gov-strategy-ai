 ...
 * v3: прицельные запросы competition под статьи-анонсы контрактов и по именам
...
const BLOCK_QUERIES: Record<BlockKind, (region: string) => string[]> = {
  ...
  competition: (r) => [
    // Банки-конкуренты за бизнес региона (Сбер прежде всего банк)
    `${r} ВТБ Альфа-банк ПСБ обслуживание бюджета зарплатный проект`,
    `${r} банк эквайринг госучреждения кредит региону`,
    // ИТ/цифровые вендоры
    `${r} внедрение информационной системы контракт млн руб`,
    `${r} министерство цифрового развития контракт информационная система`,
    `${r} Ростелеком БАРС БФТ 1С контракт регион информационная система`,
    `${r} цифровизация госуправления подрядчик поставщик`,
  ],
  stakeholders: (r) => [
