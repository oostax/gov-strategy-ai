 ...
 */
export function formatRegionContext(
  region: RegionProfile | null | undefined,
  options: { includeSberPortfolio?: boolean } = {},
): string {
  if (!region) {
  ...
  if (region.painPoints?.length) {
    lines.push("Известные ограничения и узкие места:");
  ...
  if (options.includeSberPortfolio) {
    lines.push("");
...
export function selectRelevantPlaybooks(profile: SessionProfile, playbooks: Playbook[]) {
  ...
  if (profile.taskType === "region_strategy") {
    slugs.add("analyst_mode");
  } else if (profile.userRole === "vice_president") {
    slugs.add("executive_mode");
  } else if (profile.userRole === "direction_head") {
    slugs.add("strategy_mode");
  }
  if (profile.userRole === "sales_lead" && profile.taskType !== "region_strategy") {
    slugs.add("sales_region_mode");
  }
  if (profile.userRole === "analyst") slugs.add("analyst_mode");
  if (profile.taskType === "sber_region_strategy") slugs.add("sales_region_mode");
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        ``,
        formatRegionContext(region, { includeSberPortfolio: profile.taskType === "sber_region_strategy" }),
        ``,
